	...
	for l in lst:
		...
		if logic_name == "HOST":
			...
			min_wake_current = CELLULAR_min_wake_current
                #elif logic_name == "BLE":
			#min_wake_current = BLE_min_wake_current
				...
				for column in additional_series:
					if str(column) in data_set.tag_name:
						if time == None:
							...
							ax.plot(np.asarray(time_array.data),
								np.asarray(data_set.data))
		...
		if should_plot_histogram and should_perform_computations:
			...
			n, bins, patches = plt.hist(wake_times, bins=range(0, max_bin,5), facecolor='g', alpha=0.75)
